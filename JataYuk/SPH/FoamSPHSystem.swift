...
final class FoamSPHSystem {
    ...
    func update(dt: Float) {
        ...
        //  • stronger/hotter recipes (higher model.rate) erupt harder.
        ...
        if let model {
            ...
            if elapsed < duration {
                ...
                let vigor = Float(min(2.0, model.rate / 0.345))
                solver.gasLift = 3 + 10 * vigor
                ...
                let plume = Float(model.peakHeightCm / 100.0)
                solver.liftCeiling = max(geometry.height + 0.02, plume)
            } else {
                solver.gasLift = 0
